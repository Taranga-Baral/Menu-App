...
//     // For every 0.1 km, the fare is 2.5
//     double fare = (distanceInKm * 10) * 2.5;
//     return fare;
//   }

//   // Function to get the distance from the API and calculate the fare
...
//                     String distance = await _getDistanceFromAPI(pickupLocation, deliveryLocation);
//                     double distanceInKm = double.tryParse(distance) ?? 0.0;

//                     // Calculate fare based on the distance
//                     double fare = _calculateFare(distanceInKm);

//                     // Show the Snackbar with all the details, including the fare
//                     _showSnackbar(
//                         'Username: $username\nEmail: $email\nPhone: $phoneNumber\nPickup Location: $pickupLocation\nDelivery Location: $deliveryLocation\nDistance: $distance km\nFare: \NPR${fare.toStringAsFixed(2)}');
//                   } else {
//                     _showSnackbar('Could not retrieve pickup or delivery location.');
//                   }
//                 },
...
class _HomePageState extends State<HomePage> {
  ...
  Future<String> _getDistanceFromAPI(String location1, String location2) async {
    String apiUrl = 'https://distance-api3.p.rapidapi.com/distance?location1=$location1&location2=$location2&unit=kilometers';
    String apiKey = 'cd3125ef15msh2caab8018e8198ap187972jsnb9ff3f522f8e'; 
    var response = await http.get(
      Uri.parse(apiUrl),
      headers: {
...
double _calculateFare(String distance) {
  ...
  double fare = (distanceInMeters / 100) * 2.4; // For every 100 meters, fare is 2.4
  return fare;
}
  ...
  Future<void> _storeDataInFirestore(Map<String, dynamic> data) async {
    String uniqueKey = FirebaseFirestore.instance.collection('trips').doc().id; // Generate unique key
        ...
        child: Stack(
          children: [
            ...
            if (!_isLoading)
              InAppWebView(
                initialUrlRequest: URLRequest(
                  url: WebUri(widget.url), // Use the map URL passed from HomePage
                ),
                initialOptions: InAppWebViewGroupOptions(
                  crossPlatform: InAppWebViewOptions(
                    javaScriptEnabled: true,
  ...
  bool? confirmed = await showDialog<bool>(
    ...
    builder: (BuildContext context) {
      return AlertDialog(
        ...
        content: Text('Are you sure you want to book a ride?'),
        actions: <Widget>[
          TextButton(
            child: Text('Cancel'),
            onPressed: () {
  ...
  if (confirmed == true) {
    ...
    // Execute JavaScript to get the location details from the web page
    String pickupLocation = await webView?.evaluateJavascript(source: """
      document.getElementById('route_from').value
    """) ?? 'N/A';

    String deliveryLocation = await webView?.evaluateJavascript(source: """
      document.getElementById('route_to').value
    """) ?? 'N/A';
